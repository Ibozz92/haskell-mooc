...
totalPrice (MkShoppingEntry _name price amount) = price*(fromIntegral amount)
...
data Person = Birth {age :: Int, name :: String}
  deriving Show

-- fred is a person whose name is Fred and age is 90
fred :: Person
fred = Birth {name = "Fred", age = 90}
...
setName newname p = Birth {name = newname, age = age p}

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge newage p = Birth {name = name p, age = newage}
...
data Position = Plot {xpos :: Int, ypos :: Int}

-- origin is a Position value with x and y set to 0
origin :: Position
origin = Plot {xpos = 0, ypos = 0}
...
up p = Plot {xpos = xpos p, ypos = ypos p + 1}

-- right increases the x value of a position by one
right :: Position -> Position
right p = Plot {xpos = xpos p + 1, ypos = ypos p}
...
get :: UpDown -> Int
get (Increasing ud) = ud
get (Decreasing ud) = ud
...
rgb (Mix a b) = doubleavg (rgb a) (rgb b)

doubleavg :: [Double] -> [Double] -> [Double]
doubleavg [] a = a
doubleavg a [] = a
doubleavg (x:xs) (y:ys) = (x+y) / 2 : doubleavg xs ys
...
toNat z
  |z<0 = Nothing
  |z==0 = Just Zero
  |otherwise = case toNat (z-1) of Just a -> Just (PlusOne a)
                                   Nothing -> Nothing
