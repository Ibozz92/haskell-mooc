...
greetText a
    |T.length a > 15 = T.pack"Hello, "<>(T.take 15 a)<>T.pack"...!"
    |otherwise = T.pack"Hello, "<>a<>T.pack"!"
...
shout :: T.Text -> T.Text
shout a = hShout (T.words a) True

hShout :: [T.Text] -> Bool -> T.Text
hShout [] _ = mempty
hShout (a:as) b
    |b = (T.map toUpper a)<>(if b2 then T.pack" "<>hShout as False else T.pack "")
    |otherwise = a<>(if b2 then T.pack" "<>hShout as True else T.pack "")
    where b2 = not (null as)
...
longestRepeat a = case T.uncons a of 
                  Nothing -> 0
                  Just (s,r) -> hLongestRepeat r s 0 0

hLongestRepeat :: T.Text -> Char -> Int -> Int -> Int
hLongestRepeat thetext prevchr longest current = 
    case T.uncons thetext of 
        Nothing -> longest+1
        Just (start,rest) -> let newcurrent = if prevchr == start then (current+1) else 0 in
                        hLongestRepeat rest start (max longest newcurrent) newcurrent
...
takeStrict n tex = TL.toStrict (TL.take n tex)
...
byteRange a
    |B.length a == 0 = 0
    |otherwise = B.maximum a - B.minimum a
...
xorChecksum as = case B.uncons as of
                Nothing -> 0
                (Just (a,b)) -> Data.Bits.xor a (xorChecksum b)
...
countUtf8Chars a = case decodeUtf8' a of 
                   (Left _) -> Nothing
...
pingpong s = let l = B.unpack s in BL.pack (cycle(l ++ reverse l))
