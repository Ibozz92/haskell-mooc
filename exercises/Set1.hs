...
quadruple x = double (double x) -- Hello?
...
checkPassword password = if elem password ["swordfish", "mellon"]
                         then "You're in."
                         else "ACCESS DENIED!"
...
sumTo n = if n>0 then n + sumTo (n-1) else 0
