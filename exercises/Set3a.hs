...
firstHalf s = take (length s `ediv` 2) s where
    ediv a b = if a `mod` b == 0 then a `div` b else a `div` b + 1

palindrome :: String -> Bool
palindrome s
    |null s = True
    |length s == 1 = True
    |otherwise = (take 1 s == drop (length s - 1) s) && (palindrome $ drop 1 (take (length s - 1) s))
...
capitalizeFirst (s:sl) = toUpper s : sl
...
powers :: Int -> Int -> [Int]
powers k max = map (\x -> k^x) (takeWhile (g) [0..max]) where g x = k^x <= max
