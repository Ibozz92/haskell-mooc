...
count val = foldr (\x y -> if x==val then y+1 else y) 0
...
inBoth firs seco = foldr (\x y -> if x `elem` seco then x:y else y) [] firs
...
instance Foldable List where
  foldr _ defaul Empty = defaul
  foldr func defaul (LNode a b) = func a (foldr func defaul b)
...
instance Foldable TwoList where
  foldr _ defaul TwoEmpty = defaul
  foldr func defaul (TwoNode a b c) = func a (func b (foldr func defaul c))
