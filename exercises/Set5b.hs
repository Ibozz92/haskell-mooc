...
walk (x:xs) (Node a l r) = case x of StepL -> walk xs l
                                     StepR -> walk xs r
------------------------------------------------------------------------------
...
set (x:xs) val (Node a l r) = case x of StepL -> Node a (set xs val l) r 
                                        StepR -> Node a l (set xs val r)
...
myownisjust :: Maybe a -> Bool
myownisjust Nothing = False
myownisjust _ = True
...
search x (Node a l r)
  |a==x = Just []
  |myownisjust leftseq = Just (StepL : ((\(Just a) -> a)leftseq))
  |myownisjust rightseq = Just (StepR : ((\(Just a) -> a)rightseq))
  |otherwise = Nothing
  where leftseq = search x l
        rightseq = search x r