...
member thing (Set thelist) = elem thing thelist

-- add a member to a set
add :: Ord a => a -> Set a -> Set a
add a (Set []) = Set [a]
add thing (Set (x:xs))
  |member thing (Set (x:xs)) = (Set (x:xs))
  |thing<x = Set (thing:x:xs)
  |otherwise = Set(x:toList(add thing (Set xs)))
...
data State = Start | EggsIn | SugarIn | FlourIn | ReadyForMix | Mixed | Error | Finished
  deriving (Eq,Show)

step :: State -> Event -> State
step Finished _ = Finished
step Start AddEggs = EggsIn
step EggsIn AddFlour = FlourIn
step EggsIn AddSugar = SugarIn
step FlourIn AddSugar = ReadyForMix
step SugarIn AddFlour = ReadyForMix
step ReadyForMix Mix = Mixed
...
reverseNonEmpty (x:|xs) = (\(Just a) -> a)(nonEmpty(reverse (x:xs)))

copiednonEmpty :: [a] -> Maybe (NonEmpty a)
copiednonEmpty [] = Nothing
copiednonEmpty (x:xs) = Just (x :| xs)
...
passwordAllowed pass (MinimumLength a) = length pass >= a
passwordAllowed pass (ContainsSome []) = False
passwordAllowed pass (ContainsSome (x:xs)) = elem x pass || passwordAllowed pass (ContainsSome xs)
passwordAllowed pass (DoesNotContain a) = not (passwordAllowed pass (ContainsSome a))
...
data Arithmetic = Anumber Integer | TBE String Arithmetic Arithmetic
  deriving Show
...
evaluate (Anumber i) = i
evaluate (TBE "+" a b) = (evaluate a) + (evaluate b)
evaluate (TBE "*" a b) = (evaluate a) * (evaluate b)

render :: Arithmetic -> String
render (Anumber i) = show i
render (TBE op a b) = "(" ++ (render a) ++ op ++ (render b) ++ ")"
