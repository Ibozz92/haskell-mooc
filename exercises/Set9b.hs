...
prettyPrint s = hPrettyPrint 1 1 s

hPrettyPrint :: Int -> Int -> Size -> [Coord] -> String
hPrettyPrint therow thecol totsize coords
    |therow > totsize = ""
    |elem (therow,thecol) coords = 'Q':future therow thecol totsize coords
    |otherwise = '.':future therow thecol totsize coords
    where future therow thecol totsize coords
            |thecol == totsize = '\n':hPrettyPrint (therow+1) 1 totsize coords
            |otherwise = hPrettyPrint therow (thecol+1) totsize coords
...
danger _ [] = False 
danger a (x:xs) = onedanger a x || danger a xs

onedanger :: Candidate -> Coord -> Bool
onedanger a b = sameRow a b || sameCol a b || sameDiag a b || sameAntidiag a b
...
prettyPrint2 :: Size -> Stack -> String
prettyPrint2 s = hPrettyPrint2 1 1 s

hPrettyPrint2 :: Int -> Int -> Size -> [Coord] -> String
hPrettyPrint2 therow thecol totsize coords
    |therow > totsize = ""
    |elem (therow,thecol) coords = 'Q':future therow thecol totsize coords
    |danger (therow,thecol) coords = '#':future therow thecol totsize coords
    |otherwise = '.':future therow thecol totsize coords
    where future therow thecol totsize coords
            |thecol == totsize = '\n':hPrettyPrint2 (therow+1) 1 totsize coords
            |otherwise = hPrettyPrint2 therow (thecol+1) totsize coords
...
fixFirst n ((r,c):xs)
    |c>n = Nothing
    |danger (r,c) xs = fixFirst n ((r,c+1):xs)
...
continue ((r,c):xs) = (r+1,1):(r,c):xs

backtrack :: Stack -> Stack
backtrack ((r1,c1):(r2,c2):xs) = (r2,c2+1):xs
backtrack _ = []
...
step s st = case fixFirst s st of
            (Just a) -> continue a
            Nothing -> backtrack st
...
finish s (rc:rcs)
    |length thestep > s = (\(x:xs) -> xs) thestep
    |otherwise = finish s thestep
    where thestep = step s (rc:rcs)
finish _ _ = []
